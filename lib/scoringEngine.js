...
//
// Moteur statistique de prédiction, basé sur la loi de Poisson.
//
 ...
 * derniers matchs terminés, en distinguant domicile/extérieur quand c'est
 * pertinent.
 *
 ...
 */
export function computeTeamForm(recentMatches, teamId) {
  ...
  return {
    avgScored: scored / usable.length,
    avgConceded: conceded / usable.length,
 ...
 * de données réellement disponibles pour ce match : plus l'historique de
 * forme et le H2H sont fournis, plus la confiance est élevée. Ne descend
 * jamais en dessous d'un plancher pour rester utilisable côté produit.
 */
function computeConfidence({ homeSampleSize, awaySampleSize, h2hSampleSize }) {
  ...
  return Math.round(CONFIDENCE_FLOOR + rawScore * (CONFIDENCE_CEILING - CONFIDENCE_FLOOR));
}
...
export function generatePrediction({
  ...
  const homeForm = computeTeamForm(homeRecentMatches, homeTeamId);
  const awayForm = computeTeamForm(awayRecentMatches, awayTeamId);
